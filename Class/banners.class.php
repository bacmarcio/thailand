			...
			if(isset($_POST['acao']) && $_POST['acao'] == 'addBanner') {
				...
				$lado_texto = filter_input(INPUT_POST, 'lado_texto', FILTER_SANITIZE_STRING);
					...
					try{
						...
						$sql = "INSERT INTO tbl_sliders (foto, titulo1, titulo2, breve, tem_botao, nome_botao, link_botao, lado_texto, foto_mobile) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";   
						$stm = $this->pdo->prepare($sql);   
						...
						$stm->bindValue(9, upload('foto_mobile', $pastaArquivos, 'N'));
						$stm->execute(); 
		...
		function editar($redireciona='banners.php') {
			if(isset($_POST['acao']) && $_POST['acao'] == 'editaBanner') {
				...
				try { 
					...
					$sql = "UPDATE tbl_sliders SET foto=?, titulo1=?, titulo2=?, breve=?, tem_botao=?, nome_botao=?, link_botao=?, lado_texto=?, foto_mobile=? WHERE id=?";   
					$stm = $this->pdo->prepare($sql);   
					$stm->bindValue(1, upload('foto', $pastaArquivos, 'N'));   
					$stm->bindValue(2, $titulo1);   
					$stm->bindValue(3, $titulo2);   
