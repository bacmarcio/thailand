			...
			if(isset($_POST['acao']) && $_POST['acao'] == 'addTreinamento') {
				...
				$embed = filter_input(INPUT_POST, 'embed', FILTER_SANITIZE_STRING);
					try{
						...
						$sql = "INSERT INTO tbl_treinamentos (foto, titulo, postado_por, conteudo, data_postagem, especialidade, breve, meta_title, meta_keywords, meta_description, foto1, url_amigavel, descricao_imagem, legenda_imagem, embed) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";   
						$stm = $this->pdo->prepare($sql);   
		...
		function editar($redireciona='treinamentos.php') {
			if(isset($_POST['acao']) && $_POST['acao'] == 'editaTreinamento') {
				...
				try { 
					...
					$sql = "UPDATE tbl_treinamentos SET foto=?, titulo=?, postado_por=?, conteudo=?, data_postagem=?, especialidade=?, breve=?, meta_title=?, meta_keywords=?, meta_description=?, foto1=?, url_amigavel=?, descricao_imagem=?, legenda_imagem=?, embed=? WHERE id=?";   
					$stm = $this->pdo->prepare($sql);   
					$stm->bindValue(1, upload('foto', $pastaArquivos, 'N'));   
					$stm->bindValue(2, $titulo);   
					$stm->bindValue(3, $postado_por);
