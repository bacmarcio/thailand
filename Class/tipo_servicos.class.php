<?php
@ session_start();
$TipoServicosInstanciada = '';
if(empty($TipoServicosInstanciada)) {
	...
	class TipoServicos {
		...
		private static $TipoServicos = null; 
		...
		public static function getInstance($conexao){   
			if (!isset(self::$TipoServicos)):    
				self::$TipoServicos = new TipoServicos($conexao);   
			endif;
			return self::$TipoServicos;    
		}
		...
		function rsDados($id='', $orderBy='', $limite='', $idCat='') {
			...
			try{   
				$sql = "SELECT * FROM tbl_tipo_servico where 1=1 $sql $sqlOrdem $sqlLimite";
		...
		function add($redireciona='') {
			if(isset($_POST['acao']) && $_POST['acao'] == 'addTipoServicos') {
				...
				$urlAmigavel = gerarTituloSEO($titulo);
					...
					try{
						...
						$sql = "INSERT INTO tbl_tipo_servico (foto, titulo, titulo_en, titulo_th, descricao, descricao_en, descricao_th, id_cat, meta_title, meta_keywords, meta_description, url_amigavel) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";   
						$stm = $this->pdo->prepare($sql);   
					...
					echo "	<script>
								window.location='tipo-servicos.php';
								</script>";
								exit;
		...
		function editar($redireciona='tipo-servicos.php') {
			if(isset($_POST['acao']) && $_POST['acao'] == 'editaTipoServicos') {
				...
				try { 
					...
					$sql = "UPDATE tbl_tipo_servico SET foto=?, titulo=?, titulo_en=?, titulo_th=?, descricao=?, descricao_en=?, descricao_th=?, id_cat=?, meta_title=?, meta_keywords=?, meta_description=?, url_amigavel=? WHERE id=?";   
					$stm = $this->pdo->prepare($sql);   
					$stm->bindValue(1, upload('foto', $pastaArquivos, 'N'));   
					$stm->bindValue(2, $titulo);   
					$stm->bindValue(3, $tituloEn);   
		...
		function excluir() {
			if(isset($_GET['acao']) && $_GET['acao'] == 'excluirTipoServicos') {
				
				try{   
					$sql = "DELETE FROM tbl_tipo_servico WHERE id=? ";   
					$stm = $this->pdo->prepare($sql);   
