<?php
@ session_start();
$TailandiaInstanciada = '';
if(empty($TailandiaInstanciada)) {
	...
	class Tailandia {
		...
		public static function getInstance($conexao){   
			if (!isset(self::$Tailandia)):    
				self::$Tailandia = new Tailandia($conexao);   
			endif;
			return self::$Tailandia;    
		}
		...
		function rsDados($id='', $orderBy='', $limite='', $idCat='') {
			...
			try{   
				$sql = "SELECT * FROM tbl_tailandia where 1=1 $sql $sqlOrdem $sqlLimite";
		...
		function add($redireciona='') {
			if(isset($_POST['acao']) && $_POST['acao'] == 'addTailandia') {
				...
				$urlAmigavel = gerarTituloSEO($titulo);
					...
					try{
						...
						$sql = "INSERT INTO tbl_tailandia (foto, titulo, titulo_en, titulo_th, descricao, descricao_en, descricao_th, id_cat, meta_title, meta_keywords, meta_description, url_amigavel) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";   
						$stm = $this->pdo->prepare($sql);   
		...
		function editar($redireciona='tailandia.php') {
			if(isset($_POST['acao']) && $_POST['acao'] == 'editaTailandia') {
				...
				try { 
					...
					$sql = "UPDATE tbl_tailandia SET foto=?, titulo=?, titulo_en=?, titulo_th=?, descricao, descricao_en=?, descricao_th=?, id_cat=?, meta_title=?, meta_keywords=?, meta_description=?, url_amigavel=? WHERE id=?";   
					$stm = $this->pdo->prepare($sql);   
					$stm->bindValue(1, upload('foto', $pastaArquivos, 'N'));   
					$stm->bindValue(2, $titulo);   
					$stm->bindValue(3, $tituloEn);   
		...
		function excluir() {
			if(isset($_GET['acao']) && $_GET['acao'] == 'excluirTailandia') {
				
				try{   
					$sql = "DELETE FROM tbl_tailandia WHERE id=? ";   
					$stm = $this->pdo->prepare($sql);   
